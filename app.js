...
function getSectionStats(tests) {
  ...
  return { failed, passed, notRun, total: tests.length, weatherPercent, weatherEmoji };
}

function getTotalStats() {
  if (!state.data) return { total: 0, failed: 0, passed: 0, notRun: 0 };
  const allTests = state.data.sections.flatMap(s => s.tests);
  return {
    total: allTests.length,
    failed: allTests.filter(t => t.status === 'failed').length,
    passed: allTests.filter(t => t.status === 'passed').length,
    notRun: allTests.filter(t => t.status === 'not_run' || t.status === 'running').length
  };
}
...
function renderSections() {
    ...
    sectionEl.innerHTML = `
      <div class="section-header" data-section="${section.id}">
        ...
        <div class="section-meta">
          ...
          ${stats.failed > 0 
            ? `<span class="section-status has-failed">${stats.failed} failed</span>`
            : stats.notRun > 0
              ? `<span class="section-status has-not-run">${stats.notRun} not run</span>`
              : `<span class="section-status all-green">All Green</span>`
          }
        </div>
      </div>
  ...
  // Add click handlers for error previews
  document.querySelectorAll('.test-error-preview').forEach(link => {
  ...
  // Add click handlers for failing tests analysis links
  document.querySelectorAll('.failing-tests-link').forEach(link => {
    link.addEventListener('click', (e) => {
      e.stopPropagation();
      const testId = link.dataset.testId;
      const sectionId = link.dataset.sectionId;
      showFailingTestsModal(sectionId, testId);
    });
  });
}
    ...
    <div class="test-group ${isExpanded ? 'expanded' : ''}" data-group-id="${groupId}">
      ...
      <div class="test-group-content">
        <div class="test-table-header">
          ...
          <span class="weather-header">Weather <span class="weather-range">(oldest ← 10d → newest)</span></span>
          <span>Retried</span>
        </div>
  ...
  return `
    <div class="test-row ${test.status}">
      <div class="test-name-col">
        ...
        ${test.error ? `
          <span class="test-error-preview" data-test-id="${test.id}" data-section-id="${sectionId}">
            └─ [View Error: ${test.error.step || 'Unknown'}]
          </span>
        ` : ''}
        ${hasFailingTests ? `
          <span class="failing-tests-link" data-test-id="${test.id}" data-section-id="${sectionId}">
            └─ ⚠️ ${test.failedTestsInWeather.length} unique failing test${test.failedTestsInWeather.length > 1 ? 's' : ''} in 10 days
            <span class="failing-tests-preview">(${failingTestsPreview}${test.failedTestsInWeather.length > 2 ? '...' : ''})</span>
          </span>
        ` : ''}
      </div>
...
function updateStats() {
  ...
  document.getElementById('filter-failed-count').textContent = stats.failed;
  document.getElementById('filter-not-run-count').textContent = stats.notRun;
  document.getElementById('filter-passed-count').textContent = stats.passed;
}
...
function showWeatherModal(sectionId, testId) {
  ...
  const daysHtml = [...test.weatherHistory].reverse().map((day, index) => {
    ...
    // Get failing tests for this day
    const dayFailures = day.failureDetails?.failures || [];
      ...
      <div class="weather-day-row ${day.status}">
        ...
        <div class="weather-day-message ${day.status === 'failed' ? 'failure-note' : ''}">
          ${day.status === 'passed' 
            ...
            : day.status === 'failed' 
              ? `Failed at: ${day.failureStep || 'Unknown'}${failureCount > 0 ? ` (${failureCount} "not ok" tests)` : ''}` 
              : 'No run recorded'}
        </div>
        ${day.runId ? `
            ...
            return `
              <div class="failing-test-item">
                <div class="failing-test-header">
                  ...
                  <span class="failing-test-count">${ft.count}x in 10 days</span>
                </div>
                <div class="failing-test-dates">
                  Failed on: ${ft.dates.map(d => new Date(d).toLocaleDateString('en-US', {month: 'short', day: 'numeric'})).join(', ')}
                </div>
                ${otherJobs.length > 0 ? `
  ...
  body.innerHTML = `
    ...
    ${failedCount > 0 && !test.failedTestsInWeather?.length ? `
...
function showFailingTestsModal(sectionId, testId) {
  ...
  if (!test || !test.failedTestsInWeather || test.failedTestsInWeather.length === 0) {
  ...
  title.textContent = `${test.name} — Failing Tests Analysis`;
  
  const testsHtml = test.failedTestsInWeather.map(ft => {
    // Find if this test fails in other jobs
    const otherJobs = getOtherJobsWithSameFailure(ft.name, test.name);
  ...
  body.innerHTML = `
    <div class="failing-tests-modal-header">
      <p>Analysis of specific test failures ("not ok" tests) from the last 10 days:</p>
    </div>
